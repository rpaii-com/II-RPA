...
WebmContainer.prototype.updateBySource = function () {
    ...
    for (this.offset = 0; this.offset < this.source.length; this.offset = end) {
        ...
        let info = sections[id] || { name: 'Unknown', type: 'Unknown' };
        let ctr = WebmBase;
        switch (info.type) {
            case 'Container':
                ctr = WebmContainer;
...
WebmFile.prototype.fixDuration = function (duration) {
    let segmentSection = this.getSectionById(0x8538067);
    if (!segmentSection) {
        console.log('[fix-webm-duration] Segment section is missing');
        return false;
    }

    let infoSection = segmentSection.getSectionById(0x549a966);
    if (!infoSection) {
        console.log('[fix-webm-duration] Info section is missing');
        return false;
    }

    let timeScaleSection = infoSection.getSectionById(0xad7b1);
    if (!timeScaleSection) {
